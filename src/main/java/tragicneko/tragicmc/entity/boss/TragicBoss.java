...
public abstract class TragicBoss extends EntityMob implements IBossDisplayData {
        ...
        if (TragicConfig.allowExtraBossLoot) {
            ...
            for (int i = 0; i < amount; i++) {
                if (rand.nextBoolean()) {
                    ...
                    if (luxuryDrop != null) this.capturedDrops
                        .add(new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, luxuryDrop));
                    amt++;
                }
    ...
    public void onLivingUpdate() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.EASY && !TragicConfig.allowEasyBosses || this.posY <= -30
            || this.posY > 280) this.setDead();
        ...
        if (!this.worldObj.isRemote && TragicConfig.bossesDenyFlight) {
            ...
            for (EntityPlayerMP mp : list) {
                ...
                if (TragicConfig.allowFlight && mp.isPotionActive(TragicPotion.Flight))
                    mp.removePotionEffect(TragicPotion.Flight.id);
            }
        }
    }
        ...
        if (par1DamageSource.getEntity() != null) {
            if (par1DamageSource.getEntity() instanceof EntityPlayer && par2 >= TragicConfig.bossDamageCap) {
                ...
                if (!player.capabilities.isCreativeMode || !flag)
                    par2 = MathHelper.clamp_float(par2, 0.0F, TragicConfig.bossDamageCap);
            }
