...
public class EntityEnyvil extends TragicBoss implements IMultiPart {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(enyvilStats[0]);
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(enyvilStats[1]);
        this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
            .setBaseValue(enyvilStats[2]);
        this.getEntityAttribute(SharedMonsterAttributes.followRange)
            .setBaseValue(enyvilStats[3]);
        this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance)
            .setBaseValue(enyvilStats[4]);
    }
    ...
    public void onDeath(DamageSource src) {
        ...
        if (src.getEntity() instanceof EntityPlayerMP && TragicConfig.allowAchievements)
            ((EntityPlayerMP) src.getEntity()).triggerAchievement(TragicAchievements.enyvil);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.getAttackTarget() == null) {
            ...
            if (this.canUseNewAbility() && this.isEntityInRange(this.getAttackTarget(), 6.0F, 20.0F)
                && rand.nextInt(48) == 0
                && TragicConfig.enyvilDarkLightning) this.setLightningTicks(100);
            if (this.getLightningTicks() > 0 && this.getLightningTicks() % 20 == 0 && TragicConfig.enyvilDarkLightning)
                this.useDarkLightning();

            if (this.canUseNewAbility() && this.isEntityInRange(this.getAttackTarget(), 5.0F, 16.0F)
                && rand.nextInt(96) == 0
                && TragicConfig.enyvilDarkEnergySpray) {
            ...
            if (this.canUseNewAbility() && this.isEntityInRange(this.getAttackTarget(), 12.0F, 28.0F)
                ...
                && TragicConfig.enyvilTractorBeam) {
            ...
            if (this.ticksExisted % 60 == 0 && rand.nextInt(4) == 0
                ...
                for (int meow = 0; meow < 4 + rand.nextInt(3); meow++) {
                    ...
                    this.worldObj
                        .createExplosion(this, d0, d2, d1, rand.nextFloat() * 3.0F + 1.5F, this.getMobGriefing());
                }
            }
        }

        if (this.isCollidedHorizontally && this.getMobGriefing() && TragicConfig.enyvilDestroyBlocks)
            this.destroyBlocks();
    }
    ...
    private void updateCrystal() {
        ...
        if (this.hasCrystal()) {
            if (this.ticksExisted % 10 == 0 && this.getHealth() < this.getMaxHealth()
                && TragicConfig.enyvilRegeneration) this.heal(1.0F);
    ...
    public void createNewCrystals() {
        ...
        if (!this.worldObj.isRemote && TragicConfig.allowMobSounds)
            this.playSound("tragicmc:boss.enyvil.summon", 0.6F, 1.0F);
    ...
    public void tractorBeamEntity() {
        ...
        if (!this.worldObj.isRemote && this.getTractorBeamTicks() % 10 == 0 && TragicConfig.allowMobSounds) {
            ...
            this.getAttackTarget()
                .playSound("tragicmc:boss.enyvil.tractor", 0.6F, 1.0F);
        }
    ...
    private void useLasers() {
        ...
        if (crystal != null && !crystal.isDead) {
            if (!this.worldObj.isRemote && crystal.getDistanceToEntity(this.getAttackTarget()) <= 16.0F) {
                if (!this.worldObj.isRemote && TragicConfig.allowMobSounds)
                    crystal.playSound("tragicmc:boss.enyvil.laser", 1.0F, 1.0F);
                this.getAttackTarget()
    ...
    private void createThunderstorm() {
        for (int i = 0; i < 3 + rand.nextInt(3); i++) {
            ...
            if (TragicConfig.enyvilLightningExplosions)
    ...
    private void attackEntitiesInList(List list) {
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                if (TragicConfig.allowFear) ((EntityLivingBase) entity)
                    .addPotionEffect(new PotionEffect(TragicPotion.Fear.id, 60 + rand.nextInt(160), 1));
    ...
    public String getHurtSound() {
        return TragicConfig.allowMobSounds
            ? (this.getHurtTime() > 0 ? "tragicmc:boss.enyvil.realhurt" : "tragicmc:boss.enyvil.hurt")
            : super.getHurtSound();
    }
