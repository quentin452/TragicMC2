...
public class ModelPlague extends ModelBase {
    ...
    public ModelPlague() {
        ...
        this.shapeArray = new ModelRenderer[] { shape, shape2, shape3, shape4, shape5, shape6, shape7, shape8, shape9,
            shape10 };
    }
    ...
    public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity) {
        ...
        for (int i = 0; i < this.boxList.size(); i++) {
            shapeArray[i].rotateAngleX = ((rand.nextFloat() - rand.nextFloat()) * 0.45F)
                * this.simplifyAngle(j - f2, 10.0F);
            shapeArray[i].rotateAngleY = ((rand.nextFloat() - rand.nextFloat()) * 0.45F)
                * this.simplifyAngle(j - f2, 10.0F);
            shapeArray[i].rotateAngleZ = ((rand.nextFloat() - rand.nextFloat()) * 0.45F)
                * this.simplifyAngle(j - f2, 10.0F);
        }
    }
