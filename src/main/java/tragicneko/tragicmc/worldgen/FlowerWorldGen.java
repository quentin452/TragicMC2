...
public class FlowerWorldGen implements IWorldGenerator {
    ...
    public static Set<BiomeGenBase> allowedBiomes = Sets.newHashSet(
        new BiomeGenBase[] { BiomeGenBase.forest, BiomeGenBase.forestHills, BiomeGenBase.birchForest,
            BiomeGenBase.birchForestHills, BiomeGenBase.jungle, BiomeGenBase.jungleHills, BiomeGenBase.plains,
            BiomeGenBase.taiga, BiomeGenBase.taigaHills, BiomeGenBase.roofedForest, BiomeGenBase.savanna,
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (world.provider.dimensionId == 0 && !TragicConfig.allowExtraOverworldFlowers
            || world.provider.dimensionId != 0 && !(world.provider instanceof TragicWorldProvider)
            || !TragicConfig.allowFlowerGen) return;
        ...
        if (world.provider.dimensionId == 0) // discriminator based flower generation for the overworld
        {
            if (world.getWorldInfo()
                .getTerrainType() == WorldType.FLAT) return;

            boolean[] discrim = new boolean[16];
            Arrays.fill(discrim, true);

            discrim[14] = random.nextInt(50) == 0; // this is the stapelia, it's rare, this means there's a one in 50
                                                   // chance for a specific chunk to be able to generate a stapelia
            // the chunk then has to choose it out of all available flowers based on biome, this makes it extremely rare
            // in natural generation however it can generate in any allowed biome
            ...
            for (int i = 0; i < biome.theBiomeDecorator.flowersPerChunk * 4 && discrim[meta]; i++) {
                ...
                if (world.isAirBlock(Xcoord, Ycoord, Zcoord) && (!world.provider.hasNoSky || Ycoord < 255)
            ...
            boolean flwr = flag5 || flag6; // prevent any other biomes from generating them if they use the other flower
                                           // generator
